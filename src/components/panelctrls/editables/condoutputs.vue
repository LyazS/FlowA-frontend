<template>
    <n-card title="选择器" class="condition-selector">
        <n-space vertical>
            <!-- 连接多个下游分支的说明文字 -->
            <n-text depth="3">
                    ...
                    <n-form :model="branch" label-placement="left" label-width="80">
                        ...
                        <div v-for="(condition, cIndex) in branch.andConditions" :key="cIndex">
                            <n-grid :cols="4" :x-gap="12" style="margin-top: 12px">
                                ...
                                <n-grid-item>
                                    <n-space>
                                        <n-select v-model:value="condition.value" placeholder="请选择"
                                            style="width: 100%" />
                                        <n-button circle tertiary type="error"
                                            @click="removeAndCondition(branch, cIndex)">
                                            <template #icon>
                                                <n-icon>
                                                    <Close />
                                                </n-icon>
...
import { useMessage,NText, NIcon, NButton, NCard, NForm, NFormItem, NGrid, NGridItem, NInput, NSelect, NSpace, NTag } from 'naive-ui'
const branches = ref([
    {
        variable: 'BOT_L',
        condition: 'lessThan',
