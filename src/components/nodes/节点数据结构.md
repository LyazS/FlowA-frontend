...
### 文本打印
`{ label: "内容", type: "ArrayString", key: "text", data: [], uitype: "textprint" }`

类型为`ArrayString`，key为text，data为文本内容数组，uitype为textprint，可以打印多个文本内容
...
type里边表示是哪种代码，可以是`CodePython`、`CodeJavaScript`.
...
### LLM推理—————Prompts数组
`{ "label": "Prompts", "type": "ArrayPrompt", "key": "prompts", "data": [], uitype: "llmprompts" }`
