...
基本样式`{ label: "内容", type: "String", key: "text", data: "", uitype: "" }`
...
### 文本打印
`{ label: "内容", type: "Array<String>", key: "text", data: [], uitype: "textprint" }`

类型为`Array<String>`，key为text，data为文本内容数组，uitype为textprint，可以打印多个文本内容
...
### 封装输出数组(packoutputs)
`{ "label": "<argname>", "type": "Array<type>", "key": "<argname>", "data": "<nid>/<payloads/results>/<pid/rid>" }`

label为变量名

type为`Array<type>`，`<type>`为数组元素类型
...
```js
{
    ...
    condType: "<AND|OR>",
    conditions: [
        { refdata: "<引用变量路径>", op: "<操作符>", comparetype: "<ref|value>", value: "<如果是ref则为引用变量路径|如果是value则为具体值，要区分数值和字符串>" },
        // ...
    ]
}
```
...
`{ "label": "输入变量", "type": "CodeInput", "key": "inputvars", "data": [], uitype: "codeinputs" }`
...
`{ label: "Python 代码", type: "Code<code type>>", key: "code", data: "<code>", uitype: "codeeditor" }`

只有一条，代表代码编辑器的代码

type里边表示是哪种代码，可以是`Code<Python>`、`Code<JavaScript>`.

data里边是代码内容

### LLM推理—————输入变量
`{ "label": "输入变量", "type": "LLMInput", "key": "inputvars", "data": [], uitype: "llminputs" }`
...
### LLM推理—————Prompts数组
`{ "label": "Prompts", "type": "Array<Prompt>", "key": "prompts", "data": [], uitype: "llmprompts" }`
